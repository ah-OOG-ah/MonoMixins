...
final class SymbolTable {
      ...
      while (itemIndex < this.constantPoolCount) {
         ...
         switch (itemTag) {
            ...
            case 11: {
               ...
               this.addConstantMemberReference(
                  itemIndex,
                  itemTag,
            ...
            case 15: {
               ...
               this.addConstantMethodHandle(
                  ...
                  classReader.readUTF8(nameAndTypeItemOffset, charBuffer),
                  classReader.readUTF8(nameAndTypeItemOffset + 2, charBuffer),
                  classReader.readByte(memberRefItemOffset - 1) == 11
               );
               break;
            }
            case 17:
   ...
   private void copyBootstrapMethods(ClassReader classReader, char[] charBuffer) {
      ...
      if (this.bootstrapMethodCount > 0) {
         ...
         for (int i = 0; i < this.bootstrapMethodCount; i++) {
            ...
            this.add(new SymbolTable.Entry(i, 64, offset, hashCode & 2147483647));
         }
      }
   }
   ...
   void putBootstrapMethods(ByteVector output) {
      if (this.bootstrapMethods != null) {
         output.putShort(this.addConstantUtf8("BootstrapMethods"))
   ...
   Symbol addConstantMethodHandle(int referenceKind, String owner, String name, String descriptor, boolean isInterface) {
      ...
      return this.put(new SymbolTable.Entry(this.constantPoolCount++, 15, owner, name, descriptor, data, hashCode));
   }
   ...
   private void addConstantMethodHandle(int index, int referenceKind, String owner, String name, String descriptor, boolean isInterface) {
      ...
      int data = getConstantMethodHandleSymbolData(referenceKind, isInterface);
      int hashCode = hash(15, owner, name, descriptor, data);
      this.add(new SymbolTable.Entry(index, 15, owner, name, descriptor, data, hashCode));
   }
   ...
   private Symbol addConstantDynamicOrInvokeDynamicReference(int tag, String name, String descriptor, int bootstrapMethodIndex) {
      ...
      return this.put(new SymbolTable.Entry(this.constantPoolCount++, tag, null, name, descriptor, bootstrapMethodIndex, hashCode));
   }
   ...
   private void addConstantDynamicOrInvokeDynamicReference(int tag, int index, String name, String descriptor, int bootstrapMethodIndex) {
      int hashCode = hash(tag, name, descriptor, bootstrapMethodIndex);
      this.add(new SymbolTable.Entry(index, tag, null, name, descriptor, bootstrapMethodIndex, hashCode));
   }
   ...
   private Symbol addBootstrapMethod(int offset, int length, int hashCode) {
      ...
      return this.put(new SymbolTable.Entry(this.bootstrapMethodCount++, 64, offset, hashCode));
   }
   ...
   int addUninitializedType(String value, int bytecodeOffset) {
      ...
      return this.addTypeInternal(new SymbolTable.Entry(this.typeCount, 129, value, bytecodeOffset, hashCode));
   }
   ...
   int addForwardUninitializedType(String value, Label label) {
      ...
      for (SymbolTable.Entry entry = this.get(hashCode); entry != null; entry = entry.next) {
         if (entry.tag == 130 && entry.hashCode == hashCode && entry.data == labelIndex && entry.value.equals(value)) {
            return entry.index;
         }
      }

      return this.addTypeInternal(new SymbolTable.Entry(this.typeCount, 130, value, labelIndex, hashCode));
   }
   ...
   int addMergedType(int typeTableIndex1, int typeTableIndex2) {
      ...
      this.put(new SymbolTable.Entry(this.typeCount, 131, data, hashCode)).info = commonSuperTypeIndex;
   ...
   private SymbolTable.LabelEntry getOrAddLabelEntry(Label label) {
      ...
      SymbolTable.LabelEntry labelEntry = this.labelEntries[hashCode % this.labelEntries.length];
      ...
      if (labelEntry != null) {
         ...
         if (this.labelCount > this.labelEntries.length * 3 / 4) {
            ...
            int newCapacity = currentCapacity * 2 + 1;
            SymbolTable.LabelEntry[] newLabelEntries = new SymbolTable.LabelEntry[newCapacity];

            for (int i = currentCapacity - 1; i >= 0; i--) {
               SymbolTable.LabelEntry currentEntry = this.labelEntries[i];

               while (currentEntry != null) {
                  int newCurrentEntryIndex = System.identityHashCode(currentEntry.label) % newCapacity;
                  SymbolTable.LabelEntry nextEntry = currentEntry.next;
         ...
         if (this.labelCount == this.labelTable.length) {
            SymbolTable.LabelEntry[] newLabelTable = new SymbolTable.LabelEntry[2 * this.labelTable.length];
            System.arraycopy(this.labelTable, 0, newLabelTable, 0, this.labelTable.length);
            this.labelTable = newLabelTable;
         }

         labelEntry = new SymbolTable.LabelEntry(this.labelCount, label);
