...
class Frame {
   ...
   static int getAbstractTypeFromApiFormat(SymbolTable symbolTable, Object type) {
      if (type instanceof Integer) {
         ...
         return (label.flags & 4) != 0
            ? 12582912 | symbolTable.addUninitializedType("", label.bytecodeOffset)
            : 16777216 | symbolTable.addForwardUninitializedType("", label);
      }
   }
         ...
         case '[':
            ...
            switch (buffer.charAt(elementDescriptorOffset)) {
               ...
               case 'J':
                  ...
                  break;
               case 'L':
                  String internalNamex = buffer.substring(elementDescriptorOffset + 1, buffer.length() - 1);
                  typeValue = 8388608 | symbolTable.addType(internalNamex);
                  break;
               case 'S':
                  typeValue = 4194316;
                  break;
   ...
   final void setInputFrameFromApiFormat(SymbolTable symbolTable, int numLocal, Object[] local, int numStack, Object[] stack) {
      ...
      for (int ix = 0; ix < numStack; ix++) {
         if (stack[ix] == Opcodes.LONG || stack[ix] == Opcodes.DOUBLE) {
            numStackTop++;
         }
      }
      ...
      for (int ixx = 0; ixx < numStack; ixx++) {
         this.inputStack[inputStackIndex++] = getAbstractTypeFromApiFormat(symbolTable, stack[ixx]);
         if (stack[ixx] == Opcodes.LONG || stack[ixx] == Opcodes.DOUBLE) {
