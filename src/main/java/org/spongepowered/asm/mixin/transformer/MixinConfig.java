...
final class MixinConfig implements Comparable<MixinConfig>, IMixinConfig {
   ...
   private void initCompatibilityLevel() {
      ...
      if (this.compatibility != null) {
         ...
         try {
            ...
            throw new MixinInitialisationError(
               String.format("Mixin config %s specifies compatibility level %s which is not recognised", this.name, strCompatibility)
            );
         }
         ...
         if (this.compatibilityLevel != currentLevel) {
            if (currentLevel.isAtLeast(this.compatibilityLevel) && !currentLevel.canSupport(this.compatibilityLevel)) {
               ...
               throw new MixinInitialisationError(
                  String.format("Mixin config %s requires compatibility level %s which is prohibited by %s", this.name, this.compatibilityLevel, currentLevel)
               );
            } else {
   ...
   void checkCompatibilityLevel(MixinInfo mixin, int majorVersion, int minorVersion) {
      if (majorVersion > this.compatibilityLevel.getClassMajorVersion()) {
         ...
         String message = majorVersion > MixinEnvironment.CompatibilityLevel.MAX_SUPPORTED.getClassMajorVersion()
      ...
      try {
         ...
         if (injectionPointClass != null) {
            try {
               ...
               this.logger.error("Unable to register injection point {} for {}, the class is not compatible with this version of Mixin", className, this, var5);
               return;
            }
   ...
   private Class<?> findExtensionClass(String className, Class<?> superType, String extensionType) {
      ...
      try {
         ...
         this.logger.error("Unable to register {} {} for {}, the specified class was not found", extensionType, className, this, var6);
         return null;
      }
   ...
   private boolean checkVersion() throws MixinInitialisationError {
      ...
      if (minVersion.compareTo(curVersion) > 0) {
         this.logger
            .warn(
               "Mixin config {} requires mixin subsystem version {} but {} was found. The mixin config will not be applied.", this.name, minVersion, curVersion
            );
         if (this.required) {
   ...
   private boolean checkFeatures() throws MixinInitialisationError {
      if (this.requiredFeatures != null && !this.requiredFeatures.isEmpty()) {
         ...
         if (missingFeatures.isEmpty()) {
            ...
            this.logger
               .warn("Mixin config {} requires features [{}] which are not available. The mixin config will not be applied.", this.name, strMissingFeatures);
            if (this.required) {
               throw new MixinInitialisationError(
                  "Required mixin config " + this.name + " requires features [" + strMissingFeatures + " which are not available"
               );
            } else {
               return false;
            }
      ...
      if (!Strings.isNullOrEmpty(this.mixinPackage)) {
         ...
         if (this.refMapperWrapper != null) {
            ...
            try {
               ...
               this.logger.error("Reference map wrapper '{}' could not be found: ", wrapperName, var5);
            } catch (ReflectiveOperationException var6) {
               this.logger.error("Reference map wrapper '{}' could not be created: ", wrapperName, var6);
            } catch (SecurityException var7) {
               this.logger.error("Reference map wrapper '{}' could not be created: ", wrapperName, var7);
            }
         }
      }
   }
      ...
      if (mixinClasses != null) {
         if (Strings.isNullOrEmpty(this.mixinPackage)) {
            if (mixinClasses.size() > 0) {
               this.logger
                  .error(
                     "{} declares mixin classes in {} but does not specify a package, {} orphaned mixins will not be loaded: {}",
                     this,
                     collectionName,
                     mixinClasses.size(),
                     mixinClasses
                  ...
                  try {
                     ...
                     if (this.required) {
                        throw new InvalidMixinException(
                           mixin, "Error initialising mixin " + mixin + " - " + var13.getClass() + ": " + var13.getMessage(), var13
                        );
                     }
   ...
   String remapClassName(String className, String reference) {
      ...
      return mapper instanceof IClassReferenceMapper
         ? ((IClassReferenceMapper)mapper).remapClassName(className, reference)
         : mapper.remap(className, reference);
   }
   ...
   static Config create(String configFile, MixinEnvironment outer, IMixinConfigSource source) {
      try {
         ...
         throw new IllegalArgumentException(String.format("The specified resource '%s' was invalid or could not be read", configFile), var11);
      }
   }
