...
public class MixinTargetContext extends ClassContext implements IMixinContext {
   ...
   private Object transformConstant(MethodNode method, Iterator<AbstractInsnNode> iter, Object constant) {
      if (constant instanceof Type) {
         ...
         return ASM.isAtLeastVersion(6) && constant instanceof ConstantDynamic
            ? this.transformDynamicConstant(method, iter, (ConstantDynamic)constant)
            : constant;
      }
   }
   ...
   private ConstantDynamic transformDynamicConstant(MethodNode method, Iterator<AbstractInsnNode> iter, ConstantDynamic constant) {
      ...
      if (!MixinEnvironment.getCompatibilityLevel().supports(16)) {
         throw new InvalidMixinException(
            this,
            String.format("%s%s in %s contains a dynamic constant, which is not supported by the current compatibility level", method.name, method.desc, this)
         );
      } else {
   ...
   protected void requireVersion(int version) {
      ...
      int minorVersion = version >> 16 & 65535;
      if (majorVersion > (this.minRequiredClassVersion & 65535)) {
         this.minRequiredClassVersion = version;
         if (majorVersion > ASM.getMaxSupportedClassVersionMajor()) {
            throw new InvalidMixinException(
               this, String.format("Unsupported mixin class version %d.%d. ASM supports %s", majorVersion, minorVersion, ASM.getClassVersionString())
            );
         } else {
   ...
   void postApply(String transformedName, ClassNode targetClass) {
      ...
      try {
         ...
         throw new InvalidInjectionException(
            group.getMembers().iterator().next().getMixin(),
   ...
   void applyInjections(int injectorOrder) {
      ...
      try {
         ...
         for (InjectionInfo injectInfox : injectors) {
            injectActivity.next(injectInfox.toString());
            injectInfox.inject();
         }
         ...
         for (InjectionInfo injectInfox : injectors) {
            postInjectActivity.next(injectInfox.toString());
            injectInfox.postInject();
         }
