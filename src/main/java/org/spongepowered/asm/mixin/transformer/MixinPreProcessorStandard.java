...
class MixinPreProcessorStandard {
      ...
      if (this.prepared) {
         ...
         try {
            ...
            for (MixinInfo.MixinMethodNode mixinMethod : this.classNode.mixinMethods) {
               ClassInfo.Method method = this.mixin.getClassInfo().findMethod(mixinMethod, mixinMethod.access | 262144);
   ...
   protected void prepareInnerClasses(Extensions extensions) {
      InnerClassGenerator icg = extensions.getGenerator(InnerClassGenerator.class);
         ...
         if (MixinEnvironment.getCompatibilityLevel().isAtLeast(MixinEnvironment.CompatibilityLevel.JAVA_8) && method.isStatic()) {
            ...
            if (method.isConformed()) {
               ...
               logger.log(
                  ...
                  Annotations.getSimpleName(annotation),
                  mixinMethod.name,
                  mixinMethod.desc,
                  uniqueName,
                  this.mixin
         ...
         if (target == null && !synthetic) {
            ...
            if (Bytecode.getVisibility(mixinMethod).ordinal() < Bytecode.Visibility.PUBLIC.ordinal()) {
               if (method.isConformed()) {
                  ...
                  logger.log(
                     this.mixin.getLoggingLevel(), "Renaming {} method {}{} to {} in {}", type, mixinMethod.name, mixinMethod.desc, uniqueName, this.mixin
                  );
                  mixinMethod.name = method.conform(uniqueName);
               }
