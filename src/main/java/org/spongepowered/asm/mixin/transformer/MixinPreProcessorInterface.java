...
class MixinPreProcessorInterface extends MixinPreProcessorStandard {
   ...
   protected void prepareMethod(MixinInfo.MixinMethodNode mixinMethod, ClassInfo.Method method) {
      ...
      if (!isPublic && mixinMethod.isSynthetic() && mixinMethod.isSynthetic()) {
         ...
         if (injectorAnnotation == null) {
            ...
            throw new InvalidInterfaceMixinException(
               this.mixin,
               String.format("Interface mixin contains an injector but Feature.INJECTORS_IN_INTERFACE_MIXINS is disabled! Found %s in %s", method, this.mixin)
            );
         } else {
   ...
   protected boolean validateField(MixinTargetContext context, FieldNode field, AnnotationNode shadow) {
      if (!Bytecode.isStatic(field) || !Bytecode.hasFlag(field, 1) || !Bytecode.hasFlag(field, 16)) {
         throw new InvalidInterfaceMixinException(
            this.mixin, String.format("Interface mixin contains an illegal field! Found %s %s in %s", Modifier.toString(field.access), field.name, this.mixin)
         );
      } else if (shadow == null) {
         ...
         throw new InvalidInterfaceMixinException(
            this.mixin, String.format("@Shadow field %s.%s is marked as mutable. This is not allowed.", this.mixin, field.name)
         );
      } else {
