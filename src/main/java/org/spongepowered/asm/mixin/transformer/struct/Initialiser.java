...
public class Initialiser {
   ...
   private void initInstructions(InsnRange range) {
      ...
      if (last != null && last.getOpcode() != 181) {
         throw new InvalidMixinException(
            this.mixin, "Could not parse initialiser, expected 0xB5, found 0x" + Integer.toHexString(last.getOpcode()) + " in " + this
         );
      }
   }
   ...
   public void injectInto(Constructor ctor) {
      ...
      if (marker == null) {
         ...
         for (AbstractInsnNode nodex : this.insns) {
            if (nodex instanceof LabelNode) {
            }

            if (nodex instanceof JumpInsnNode) {
            }

            ctor.insertBefore(marker, nodex.clone(labels));
         }
      }
   }
   ...
   public static enum InjectionMode {
      ...
      public static Initialiser.InjectionMode ofEnvironment(MixinEnvironment env) {
