...
class InvokerInfo extends AccessorInfo {
   ...
   private MethodNode findTargetMethod() {
      TargetSelector.Result<MethodNode> result = TargetSelector.run(
         this.target.configure(ITargetSelector.Configure.ORPHAN), ElementNode.methodList(this.classNode)
      );

      try {
         return result.getSingleResult(true);
      } catch (IllegalStateException var5) {
         String message = String.format("%s matching %s in %s for %s", var5.getMessage(), this.target, this.classNode.name, this);
