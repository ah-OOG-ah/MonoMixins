   ...
   public String remapWithContext(String context, String className, String reference) {
      if (reference.isEmpty()) {
         ...
         if (remappedCached != null) {
            ...
            MemberInfo info = MemberInfo.parse(origInfoString, null);
            if (info.getName() == null && info.getDesc() == null) {
               ...
               if (info.isField()) {
                  remapped = new MemberInfo(
                        this.remapper.mapFieldName(info.getOwner(), info.getName(), info.getDesc()),
                        info.getOwner() == null ? null : this.remapper.map(info.getOwner()),
                        info.getDesc() == null ? null : this.remapper.mapDesc(info.getDesc())
                     )
                     .toString();
               } else {
                  remapped = new MemberInfo(
                        this.remapper.mapMethodName(info.getOwner(), info.getName(), info.getDesc()),
                        info.getOwner() == null ? null : this.remapper.map(info.getOwner()),
                        info.getDesc() == null ? null : remapMethodDescriptor(this.remapper, info.getDesc())
                     )
                     .toString();
               }
