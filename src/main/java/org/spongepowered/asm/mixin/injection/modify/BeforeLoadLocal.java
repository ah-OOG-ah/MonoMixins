...
public class BeforeLoadLocal extends ModifyVariableInjector.LocalVariableInjectionPoint {
   ...
   boolean find(InjectionInfo info, InsnList insns, Collection<AbstractInsnNode> nodes, Target target) {
      ...
      for (AbstractInsnNode insn : FabricUtil.getCompatibility(info) >= 10000 ? insns : target.method.instructions) {
   ...
   class SearchState {
      ...
      void check(InjectionInfo info, Target target, Collection<AbstractInsnNode> nodes, AbstractInsnNode insn) {
         LocalVariableDiscriminator.Context context = new LocalVariableDiscriminator.Context(
            info, BeforeLoadLocal.this.returnType, BeforeLoadLocal.this.discriminator.isArgsOnly(), target, insn
         );
         int local = -2;
