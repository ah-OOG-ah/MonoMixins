...
public class BeforeInvoke extends InjectionPoint {
   ...
   public BeforeInvoke(InjectionPointData data) {
      ...
      this.allowPermissive = this.mixin.getOption(MixinEnvironment.Option.REFMAP_REMAP)
         && this.mixin.getOption(MixinEnvironment.Option.REFMAP_REMAP_ALLOW_PERMISSIVE)
   ...
   protected boolean find(String desc, InsnList insns, Collection<AbstractInsnNode> nodes, ITargetSelector selector, BeforeInvoke.SearchType searchType) {
      if (selector == null) {
         ...
         ITargetSelector target = (searchType == BeforeInvoke.SearchType.PERMISSIVE ? selector.configure(ITargetSelector.Configure.PERMISSIVE) : selector)
            .configure(ITargetSelector.Configure.SELECT_INSTRUCTION);
         int ordinal = 0;
         int found = 0;
         int matchCount = 0;
         ...
         if (matchCount < target.getMinMatchCount()) {
            throw new SelectorConstraintException(
               target,
               String.format("%s did not match the required number of targets (required=%d, matched=%d)", target, selector.getMinMatchCount(), matchCount)
            );
         } else {
            return found > 0;
         }
