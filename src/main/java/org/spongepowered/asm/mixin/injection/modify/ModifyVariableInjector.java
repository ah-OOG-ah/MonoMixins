...
public class ModifyVariableInjector extends Injector {
   ...
   protected boolean findTargetNodes(InjectorTarget target, InjectionPoint injectionPoint, Collection<AbstractInsnNode> nodes) {
      ...
      boolean found = injectionPoint instanceof ModifyVariableInjector.LocalVariableInjectionPoint
         ? ((ModifyVariableInjector.LocalVariableInjectionPoint)injectionPoint).find(this.info, slice, nodes, target.getTarget())
   ...
   protected void preInject(Target target, InjectionNodes.InjectionNode node) {
      ...
      if (!node.hasDecoration(key)) {
         ModifyVariableInjector.Context context = new ModifyVariableInjector.Context(
            this.info, this.returnType, this.discriminator.isArgsOnly(), target, node.getCurrentTarget()
         );
   ...
   protected void inject(Target target, InjectionNodes.InjectionNode node) {
      if (node.isReplaced()) {
         ...
         ModifyVariableInjector.Context context = node.getDecoration(this.getTargetNodeKey(target, node));
         if (context == null) {
            ...
            if (this.returnType == Type.VOID_TYPE) {
               throw new InvalidInjectionException(
                  this.info,
                  String.format(
                     "%s %s method %s from %s has an invalid signature, cannot return a VOID type.", this.annotationType, handler, this, this.info.getMixin()
                  )
               );
            } else {
   ...
   abstract static class LocalVariableInjectionPoint extends InjectionPoint {
      ...
      public boolean find(String desc, InsnList insns, Collection<AbstractInsnNode> nodes) {
         if (insns instanceof IInsnListEx) {
            ...
            Target target = xinsns.getDecoration("mv.target");
            if (target != null) {
               return this.find(xinsns.getDecoration("mv.info"), insns, nodes, target);
            }
         }
