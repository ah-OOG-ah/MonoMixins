      ...
      if (min < 1) {
         ...
         if (this.minCallbackCount > 0 && this.minCallbackCount != min) {
            MixinService.getService()
               .getLogger("mixin")
               .warn("Conflicting min value '{}' on @Group({}), previously specified {}", min, this.name, this.minCallbackCount);
         }
   ...
   public void setMaxAllowed(int max) {
      if (max < 1) {
         ...
         if (this.maxCallbackCount < Integer.MAX_VALUE && this.maxCallbackCount != max) {
            MixinService.getService()
               .getLogger("mixin")
               .warn("Conflicting max value '{}' on @Group({}), previously specified {}", max, this.name, this.maxCallbackCount);
         }
