...
public class TargetSelectors implements Iterable<TargetSelectors.SelectedMethod> {
            ...
            for (MethodNode target : this.targetClassNode.methods) {
               if (passSelector.match(ElementNode.of(this.targetClassNode, target)).isExactMatch()) {
                  ...
                  if (maxCount <= 1 || (this.isStatic || !Bytecode.isStatic(target)) && target != this.method && !isMixinMethod) {
                     ...
                     this.targets.add(new TargetSelectors.SelectedMethod(passSelector, target));
                  }
   ...
   protected void findNestedTargets() {
      ...
      do {
         recursed = false;
         ListIterator<TargetSelectors.SelectedMethod> iter = this.targets.listIterator();

         while (iter.hasNext()) {
            TargetSelectors.SelectedMethod target = iter.next();
            ...
            if (next != null) {
               ...
               for (ElementNode<AbstractInsnNode> candidate : result.candidates) {
                  if (candidate.getInsn().getOpcode() == 186) {
                     ...
                     iter.add(new TargetSelectors.SelectedMethod(target, next, method));
                  }
               }
            }
         }
   ...
   public static class SelectedMethod {
      ...
      SelectedMethod(TargetSelectors.SelectedMethod parent, ITargetSelector selector, MethodNode method) {
         this.parent = parent;
