...
public class RedirectInjector extends InvokeInjector {
   ...
   protected void addTargetNode(
      InjectorTarget injectorTarget, List<InjectionNodes.InjectionNode> myNodes, AbstractInsnNode insn, Set<InjectionPoint> nominators
   ) {
   ...
   private void injectAtSetArray(RedirectInjector.RedirectedFieldData field, AbstractInsnNode varNode) {
      ...
      if (this.checkCoerce(
         valueArgIndex, elementType, String.format("%s array setter method %s from %s", this.annotationType, this, this.info.getMixin()), true
      )) {
   ...
   private void doNullCheck(InsnList insns, Target.Extension extraStack, String type, String value) {
      ...
      this.throwException(
         insns, extraStack, "java/lang/NullPointerException", String.format("%s %s %s returned null for %s", this.annotationType, type, this, value)
      );
   ...
   static class RedirectedInvokeData extends Injector.InjectorData {
      ...
      RedirectedInvokeData(Target target, MethodInsnNode node) {
         ...
         this.handlerArgs = this.isStatic ? this.targetArgs : ObjectArrays.concat(Type.getObjectType(node.owner), this.targetArgs);
      }
   }
}
