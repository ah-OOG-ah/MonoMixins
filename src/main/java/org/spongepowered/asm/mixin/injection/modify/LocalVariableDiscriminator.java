...
public class LocalVariableDiscriminator {
   ...
   public static class Context implements PrettyPrinter.IPrettyPrintable {
      ...
      private LocalVariableDiscriminator.Context.Local[] initLocals(Target target, boolean argsOnly, AbstractInsnNode node) {
         if (!argsOnly) {
            LocalVariableNode[] locals = Locals.getLocalsAt(target.classNode, target.method, node, FabricUtil.getCompatibility(this.info));
            if (locals != null) {
      ...
      private LocalVariableDiscriminator.Context.Local[] getLocals(LocalVariableNode[] initialLocals) {
         LocalVariableDiscriminator.Context.Local[] lvt = new LocalVariableDiscriminator.Context.Local[initialLocals.length];

         for (int l = 0; l < initialLocals.length; l++) {
            if (initialLocals[l] != null) {
               lvt[l] = new LocalVariableDiscriminator.Context.Local(initialLocals[l].name, Type.getType(initialLocals[l].desc));
            }
         }
