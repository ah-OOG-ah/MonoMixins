...
public final class DescriptorResolver {
   ...
   public static IResolvedDescriptor resolve(IAnnotationHandle desc, ISelectorContext context) {
      return new DescriptorResolver.Descriptor(Collections.emptySet(), desc, context);
   }

   public static IResolvedDescriptor resolve(String id, ISelectorContext context) {
      boolean debug = false;
      DescriptorResolver.IResolverObserver observer = new DescriptorResolver.ResolverObserverBasic();
      if (!Strings.isNullOrEmpty(id)) {
         if (PRINT_ID.equals(id)) {
            observer = new DescriptorResolver.ResolverObserverDebug(context);
            id = "";
            debug = true;
         } else {
            observer.visit(id, "", "");
      ...
      return new DescriptorResolver.Descriptor(observer.getSearched(), desc, context, debug);
   }

   private static IAnnotationHandle resolve(String id, ISelectorContext context, DescriptorResolver.IResolverObserver observer, String coordinate) {
   ...
   private static IAnnotationHandle resolve(
      String id, ISelectorContext context, DescriptorResolver.IResolverObserver observer, String coordinate, Object element, String detail
   ) {
   ...
   private static IAnnotationHandle resolve(
      String id, ISelectorContext context, DescriptorResolver.IResolverObserver observer, String coordinate, List<IAnnotationHandle> availableDescriptors
   ) {
   ...
   private static IAnnotationHandle resolve(
      String id, ISelectorContext context, DescriptorResolver.IResolverObserver observer, String coordinate, IAnnotationHandle desc
   ) {
      if (desc != null) {
   ...
   static final class Descriptor implements IResolvedDescriptor {
      ...
      public String getName() {
         if (this.desc == null) {
            ...
            return !value.isEmpty() ? value : this.desc.getValue("name", "");
         }
      }
      ...
      public Quantifier getMatches() {
         if (this.desc == null) {
            ...
            Integer max = this.desc != null ? this.desc.getValue("max", null) : null;
