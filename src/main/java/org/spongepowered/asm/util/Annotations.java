...
public final class Annotations {
   ...
   private static AnnotationNode getSingle(List<AnnotationNode> annotations, Class<? extends Annotation>[] annotationClasses) {
      ...
      if (foundNodes > 1) {
         throw new IllegalArgumentException(
            "Conflicting annotations found: " + Lists.<AnnotationNode, String>transform(nodes, new Function<AnnotationNode, String>() {
   ...
   public static class Handle implements IAnnotationHandle {
      ...
      public List<IAnnotationHandle> getAnnotationList(String key) {
         List<AnnotationNode> value = Annotations.getValue(this.annotation, key, false);
         List<IAnnotationHandle> list = new ArrayList<>();
         if (value != null) {
            for (AnnotationNode node : value) {
               list.add(new Annotations.Handle(node));
            }
         }
      ...
      public IAnnotationHandle getAnnotation(String key) {
         AnnotationNode value = Annotations.getValue(this.annotation, key);
         return value != null ? new Annotations.Handle(value) : null;
      }
