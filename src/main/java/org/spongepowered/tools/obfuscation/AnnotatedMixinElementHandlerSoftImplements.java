...
class AnnotatedMixinElementHandlerSoftImplements extends AnnotatedMixinElementHandler {
   ...
   public void process(AnnotationHandle implementsAnnotation) {
      if (this.mixin.remap()) {
         ...
         if (interfaces.size() < 1) {
            this.ap
               .printMessage(
                  IMessagerEx.MessageType.SOFT_IMPLEMENTS_EMPTY, "Empty @Implements annotation", this.mixin.getMixinElement(), implementsAnnotation.asMirror()
               );
         } else {
                  ...
                  try {
                     ...
                     this.ap
                        .printMessage(
                           ...
                           ((AnnotationHandle)interfaceAnnotation).asMirror()
                        );
                  }
               }
            }
   ...
   private void addInterfaceMethodMapping(Interface.Remap remap, TypeHandle iface, String prefix, MethodHandle method, String name, String desc) {
      ...
      if (obfData.isEmpty()) {
         if (remap.forceRemap()) {
            this.ap
               .printMessage(IMessagerEx.MessageType.NO_OBFDATA_FOR_SOFT_IMPLEMENTS, "No obfuscation mapping for soft-implementing method", method.getElement());
         }
      } else {
