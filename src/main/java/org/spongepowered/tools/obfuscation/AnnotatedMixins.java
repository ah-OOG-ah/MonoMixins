...
final class AnnotatedMixins implements ITokenProvider, IJavadocProvider, IMixinAnnotationProcessor, ITypeHandleProvider {
   ...
   private AnnotatedMixins(ProcessingEnvironment processingEnv) {
      ...
      this.printMessage(IMessagerEx.MessageType.INFO, "SpongePowered MIXIN Annotation Processor Version=0.8.7" + pluginVersionString);
   ...
   public void printMessage(
      IMessagerEx.MessageType type, CharSequence msg, Element element, AnnotationMirror annotation, AnnotationValue value, SuppressedBy suppressedBy
   ) {
      if (type.isEnabled()) {
         ...
         if (element != null) {
            ...
            if (occurs <= 10 && occurs >= 2) {
               for (int mask = 1; mask < 1 << occurs && element == null; mask++) {
                  int offsetx = source.length - 1;

                  for (int index = 0; offsetx >= 0; offsetx--) {
                     dest[offsetx] = source[offsetx] == '$' && (mask & 1 << index++) != 0 ? 46 : source[offsetx];
                  }
