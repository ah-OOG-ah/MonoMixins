   ...
   private void processShadows(RoundEnvironment roundEnv) {
      for (Element elem : roundEnv.getElementsAnnotatedWith(Shadow.class)) {
         ...
         if (!(parent instanceof TypeElement)) {
            this.mixins.printMessage(IMessagerEx.MessageType.ERROR, "Unexpected parent with type " + TypeUtils.getElementType(parent), elem);
         } else {
            ...
            if (elem.getKind() == ElementKind.FIELD) {
               ...
               this.mixins.printMessage(IMessagerEx.MessageType.SHADOW_ON_INVALID_ELEMENT, "Element is not a method or field", elem);
            }
         }
      }
   }
   ...
   private void processOverwrites(RoundEnvironment roundEnv) {
      for (Element elem : roundEnv.getElementsAnnotatedWith(Overwrite.class)) {
         ...
         if (!(parent instanceof TypeElement)) {
            ...
            this.mixins.printMessage(IMessagerEx.MessageType.OVERWRITE_ON_NON_METHOD_ELEMENT, "Element is not a method", elem);
         }
      }
   }
   ...
   private void processAccessors(RoundEnvironment roundEnv) {
      for (Element elem : roundEnv.getElementsAnnotatedWith(Accessor.class)) {
         ...
         if (!(parent instanceof TypeElement)) {
            ...
            this.mixins.printMessage(IMessagerEx.MessageType.ACCESSOR_ON_NON_METHOD_ELEMENT, "Element is not a method", elem);
         }
      }
   }
