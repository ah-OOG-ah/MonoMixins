...
class AnnotatedMixinElementHandlerAccessor extends AnnotatedMixinElementHandler {
         ...
         if (targetName == null) {
            ...
            for (TypeHandle target : this.mixin.getTargets()) {
               try {
                  ...
                  elem.printMessage(this.ap, IMessagerEx.MessageType.ACCESSOR_ATTACH_ERROR, var6.getMessage());
                  continue;
               }
   ...
   private void registerAccessorForTarget(AnnotatedMixinElementHandlerAccessor.AnnotatedElementAccessor elem, TypeHandle target) {
      ...
      if (elem.shouldRemap()) {
         ...
         if (obfData.isEmpty()) {
            ...
            elem.printMessage(
               this.ap, IMessagerEx.MessageType.NO_OBFDATA_FOR_ACCESSOR, "Unable to locate obfuscation mapping" + info + " for @Accessor target " + elem
            );
         } else {
   ...
   private void registerFactoryForTarget(AnnotatedMixinElementHandlerAccessor.AnnotatedElementInvoker elem, TypeHandle target) {
      ...
      if (!returnType.equals(target.toString())) {
         elem.printMessage(
            this.ap,
            IMessagerEx.MessageType.FACTORY_INVOKER_RETURN_TYPE,
            "Invalid Factory @Invoker return type, expected " + target + " but found " + returnType
         );
               ...
               if (accessorName != null) {
                  for (String prefix : AccessorInfo.AccessorType.OBJECT_FACTORY.getExpectedPrefixes()) {
                     if (prefix.equals(accessorName.prefix)
                        && ("<init>".equals(accessorName.name) || target.getSimpleName().equalsIgnoreCase(accessorName.name))) {
                        this.type = AccessorInfo.AccessorType.OBJECT_FACTORY;
                        return;
                     }
                  }
