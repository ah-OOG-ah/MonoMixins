...
class AnnotatedMixinElementHandlerInjector extends AnnotatedMixinElementHandler {
   ...
   private void registerInjectorTarget(
      AnnotatedMixinElementHandlerInjector.AnnotatedElementInjector elem, String reference, ITargetSelector targetSelector, String subject
   ) {
      try {
         targetSelector.validate();
      } catch (InvalidSelectorException var8) {
         elem.printMessage(this.ap, IMessagerEx.MessageType.TARGET_SELECTOR_VALIDATION, var8.getMessage());
      }
   ...
   private boolean registerInjector(
      ...
      if (desc == null) {
         IMessagerEx.MessageType messageType = this.mixin.isMultiTarget()
            ? IMessagerEx.MessageType.MISSING_INJECTOR_DESC_MULTITARGET
            : IMessagerEx.MessageType.MISSING_INJECTOR_DESC_SINGLETARGET;
         if (target.isSimulated()) {
            elem.printMessage(
               this.ap, IMessagerEx.MessageType.MISSING_INJECTOR_DESC_SIMULATED, elem + " target '" + reference + "' in @Pseudo mixin will not be obfuscated"
            );
         } else if (target.isImaginary()) {
            elem.printMessage(
               this.ap, messageType, elem + " target requires method signature because enclosing type information for " + target + " is unavailable"
            );
         ...
         if (obfData.isEmpty()) {
            if (!target.isSimulated()) {
               ...
               elem.addMessage(
                  targetMember.isConstructor() ? IMessagerEx.MessageType.NO_OBFDATA_FOR_CTOR : IMessagerEx.MessageType.NO_OBFDATA_FOR_TARGET,
                  "Unable to locate obfuscation mapping for " + targetName,
                  elem.getElement(),
                  elem.getAnnotation()
               );
               return false;
   ...
   public void registerInjectionPoint(AnnotatedMixinElementHandlerInjector.AnnotatedElementInjectionPoint elem, String format) {
      ...
      String targetReference = elem.getAt().getValue("target");
   ...
   protected final void remapNewTarget(
      String subject, String reference, ITargetSelector selector, AnnotatedMixinElementHandlerInjector.AnnotatedElementInjectionPoint elem
   ) {
            ...
            if (remapped.isEmpty() && !SpecialPackages.isExcludedPackage(member.toCtorType())) {
               this.ap
                  .printMessage(
                     ...
                     "Unable to locate class mapping for " + subject + " '" + target + "'",
                     elem.getElement(),
   ...
   protected final void remapReference(
      String subject, String reference, ITargetSelector selector, AnnotatedMixinElementHandlerInjector.AnnotatedElementInjectionPoint elem
   ) {
      if (selector instanceof ITargetSelectorRemappable) {
         ...
         if (!targetMember.isFullyQualified()) {
            ...
            this.ap
               .printMessage(
                  IMessagerEx.MessageType.INJECTOR_TARGET_NOT_FULLY_QUALIFIED,
                  subject + " is not fully qualified, missing " + missing,
                  elem.getElement(),
                  errorElement
               );
         } else {
            try {
                     ...
                     if (targetMember.getOwner() == null || !SpecialPackages.isExcludedPackage(targetMember.getOwner())) {
                        this.ap
                           .printMessage(
                              ...
                              "Unable to locate field mapping for " + subject + " '" + reference + "'",
                              elem.getElement(),
                              errorElement,
                              SuppressedBy.MAPPING
                           );
                     ...
                     if (targetMember.getOwner() == null || !SpecialPackages.isExcludedPackage(targetMember.getOwner())) {
                        this.ap
                           .printMessage(
                              ...
                              "Unable to locate method mapping for " + subject + " '" + reference + "'",
                              elem.getElement(),
                              errorElement,
                              SuppressedBy.MAPPING
                           );
               ...
               this.ap
                  .printMessage(
                     IMessagerEx.MessageType.INJECTOR_MAPPING_CONFLICT,
                     "Unexpected reference conflict for " + subject + ": " + reference + " -> " + var8.getNew() + " previously defined as " + var8.getOld(),
                     elem.getElement(),
                     errorElement
                  );
               return;
   ...
   static class AnnotatedElementInjectionPoint extends AnnotatedMixinElementHandler.AnnotatedElementExecutable {
      ...
      public AnnotatedElementInjectionPoint(
         ExecutableElement element, AnnotationHandle inject, IMixinContext context, String selectorCoordinate, AnnotationHandle at, InjectorRemap state
      ) {
   ...
   static class AnnotatedElementSliceInjectionPoint extends AnnotatedMixinElementHandlerInjector.AnnotatedElementInjectionPoint {
      ...
      public AnnotatedElementSliceInjectionPoint(
         ...
         AnnotationHandle inject,
         IMixinContext context,
         String selectorCoordinate,
         AnnotationHandle at,
         InjectorRemap state,
         ISelectorContext parentContext
      ) {
