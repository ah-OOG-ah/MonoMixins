...
public class TargetValidator extends MixinValidator {
      ...
      if (containsNonAccessorMethod) {
         for (TypeHandle target : targets) {
            if (target != null && target.isNotInterface()) {
               this.messager
                  .printMessage(IMessagerEx.MessageType.TARGET_VALIDATOR, "Targetted type '" + target + " of " + mixin + " is not an interface", mixin);
            }
         }
      }
   }
         ...
         if (target != null && !this.validateSuperClass(target, superClass)) {
            this.messager
               .printMessage(
                  ...
                  "Superclass " + superClass + " of " + mixin + " was not found in the hierarchy of target class " + target,
                  mixin
               );
         }
      }
   ...
   private boolean checkMixinsFor(TypeHandle targetType, TypeHandle superMixin) {
      ...
      for (String targetx : annotation.getList("targets")) {
         if (this.typeHandleProvider.getTypeHandle(targetx).isSuperTypeOf(targetType)) {
            return true;
         }
      }
