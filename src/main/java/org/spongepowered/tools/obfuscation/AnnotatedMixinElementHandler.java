...
abstract class AnnotatedMixinElementHandler {
   ...
   protected final void checkConstraints(ExecutableElement method, AnnotationHandle annotation) {
      try {
         ...
         try {
            ...
            this.ap.printMessage(IMessagerEx.MessageType.CONSTRAINT_VIOLATION, var5.getMessage(), method, annotation.asMirror());
         }
      } catch (InvalidConstraintException var6) {
         this.ap.printMessage(IMessagerEx.MessageType.INVALID_CONSTRAINT, var6.getMessage(), method, annotation.asMirror(), SuppressedBy.CONSTRAINTS);
      }
   }
         ...
         if (!target.isImaginary()) {
            ...
            if (targetMethod != null) {
               ...
               this.printMessage(
                  ...
                  "Cannot find target for " + type + " method in " + target,
                  method,
                  annotation,
                  SuppressedBy.TARGET
               );
   ...
   private void validateMethodVisibility(ExecutableElement method, AnnotationHandle annotation, String type, TypeHandle target, MethodHandle targetMethod) {
      ...
      if (visTarget != null) {
         ...
         if (visTarget.ordinal() > visMethod.ordinal()) {
            this.printMessage(
               IMessagerEx.MessageType.METHOD_VISIBILITY,
               visMethod + " " + type + " method cannot reduce " + visibility,
               method,
               annotation,
            ...
            this.printMessage(
               IMessagerEx.MessageType.METHOD_VISIBILITY,
               visMethod + " " + type + " method will upgrade " + visibility,
               method,
               annotation,
               SuppressedBy.VISIBILITY
            );
            ...
            if (targetField == null) {
               ...
               if (targetField == null) {
                  this.ap
                     .printMessage(
                        IMessagerEx.MessageType.TARGET_ELEMENT_NOT_FOUND,
                        "Cannot find target for " + type + " field in " + target,
                        field,
                        annotation.asMirror(),
                        SuppressedBy.TARGET
                     );
   ...
   protected final void validateReferencedTarget(
      AnnotatedMixinElementHandler.AnnotatedElementExecutable elem, String reference, ITargetSelector targetSelector, String subject
   ) {
            ...
            if (!target.isImaginary()) {
               ...
               if (targetMethod == null) {
                  this.ap
                     .printMessage(
                        IMessagerEx.MessageType.TARGET_ELEMENT_NOT_FOUND,
                        "Cannot find target method \"" + nameRef.getName() + nameRef.getDesc() + "\" for " + subject + " in " + target,
                        elem.getElement(),
