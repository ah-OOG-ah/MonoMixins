...
public final class MinMaxPriorityQueue<E> extends AbstractQueue<E> {
   ...
   public static MinMaxPriorityQueue.Builder<Comparable> expectedSize(int expectedSize) {
      return new MinMaxPriorityQueue.Builder<Comparable>(Ordering.natural()).expectedSize(expectedSize);
   }

   public static MinMaxPriorityQueue.Builder<Comparable> maximumSize(int maximumSize) {
      return new MinMaxPriorityQueue.Builder<Comparable>(Ordering.natural()).maximumSize(maximumSize);
   }
   ...
   MinMaxPriorityQueue.MoveDesc<E> removeAt(int index) {
      ...
      if (this.size == index) {
         ...
         if (lastElementAt == index) {
            ...
            if (lastElementAt < index) {
               return changes == null
                  ? new MinMaxPriorityQueue.MoveDesc<>(actualLastElement, toTrickle)
                  : new MinMaxPriorityQueue.MoveDesc<>(actualLastElement, changes.replaced);
            } else {
               return changes;
            }
         }
   ...
   public static final class Builder<B> {
      ...
      public <T extends B> MinMaxPriorityQueue<T> create(Iterable<? extends T> initialContents) {
         MinMaxPriorityQueue<T> queue = new MinMaxPriorityQueue(
            this, MinMaxPriorityQueue.initialQueueSize(this.expectedSize, this.maximumSize, initialContents)
         );
