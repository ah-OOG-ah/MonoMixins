...
public class TreeRangeSet<C extends Comparable<?>> extends AbstractRangeSet<C> implements Serializable {
   ...
   public Range<C> rangeContaining(C value) {
      ...
      return floorEntry != null && floorEntry.getValue().contains(value) ? floorEntry.getValue() : null;
   }
      ...
      Iterator<Entry<Cut<C>, Range<C>>> descendingEntryIterator() {
         ...
         if (positiveItr.hasNext()) {
            cut = positiveItr.peek().upperBound == Cut.aboveAll()
               ...
               : this.positiveRangesByLowerBound.higherKey(positiveItr.peek().upperBound);
         } else {
         ...
         return new AbstractIterator<Entry<Cut<C>, Range<C>>>() {
            protected Entry<Cut<C>, Range<C>> computeNext() {
               if (!backingItr.hasNext()) {
                  ...
                  return RangesByUpperBound.this.upperBoundWindow.upperBound.isLessThan(range.upperBound)
                     ? this.endOfData()
         ...
         return new AbstractIterator<Entry<Cut<C>, Range<C>>>() {
            protected Entry<Cut<C>, Range<C>> computeNext() {
               if (!backingItr.hasNext()) {
                  ...
                  Range<C> range = backingItr.next();
                  return RangesByUpperBound.this.upperBoundWindow.lowerBound.isLessThan(range.upperBound)
                     ? Maps.immutableEntry(range.upperBound, range)
                     : this.endOfData();
               }
            }
      ...
      public RangeSet<C> subRangeSet(Range<C> view) {
         if (view.encloses(this.restriction)) {
            ...
            return (RangeSet<C>)(view.isConnected(this.restriction)
               ? new TreeRangeSet.SubRangeSet(this.restriction.intersection(view))
