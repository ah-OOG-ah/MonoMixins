...
import org.spongepowered.libraries.com.google.common.base.Ticker;
import org.spongepowered.libraries.com.google.common.collect.AbstractSequentialIterator;
...
import org.spongepowered.libraries.com.google.common.util.concurrent.ExecutionError;
import org.spongepowered.libraries.com.google.common.util.concurrent.Futures;
import org.spongepowered.libraries.com.google.common.util.concurrent.ListenableFuture;
import org.spongepowered.libraries.com.google.common.util.concurrent.MoreExecutors;
import org.spongepowered.libraries.com.google.common.util.concurrent.SettableFuture;
...
class LocalCache<K, V> extends AbstractMap<K, V> implements ConcurrentMap<K, V> {
   ...
   LocalCache(CacheBuilder<? super K, ? super V> builder, @Nullable CacheLoader<? super K, V> loader) {
      ...
      this.removalNotificationQueue = (Queue<RemovalNotification<K, V>>)(this.removalListener == CacheBuilder.NullListener.INSTANCE
         ? discardingQueue()
         ...
         if (!keysToLoad.isEmpty()) {
            try {
               ...
               for (K keyx : keysToLoad) {
                  V value = newEntries.get(keyx);
                  if (value == null) {
                     throw new CacheLoader.InvalidCacheLoadException("loadAll failed to return a value for " + keyx);
                  }
         ...
         for (int i = 0; i < 3; i++) {
            ...
            for (LocalCache.Segment<K, V> segment : segments) {
               ...
               for (int j = 0; j < table.length(); j++) {
                  for (LocalCache.ReferenceEntry<K, V> e = table.get(j); e != null; e = e.getNext()) {
         ...
         try {
            ...
            if (previousValue == null) {
               ...
               return newValue == null
                  ...
                  : Futures.transform(newValue, new org.spongepowered.libraries.com.google.common.base.Function<V, V>() {
                     @Override
   ...
   static class Segment<K, V> extends ReentrantLock {
      ...
      ListenableFuture<V> loadAsync(
         final K key, final int hash, final LocalCache.LoadingValueReference<K, V> loadingValueReference, CacheLoader<? super K, V> loader
      ) {
      ...
      boolean removeEntry(LocalCache.ReferenceEntry<K, V> entry, int hash, RemovalCause cause) {
         ...
         for (LocalCache.ReferenceEntry<K, V> e = first; e != null; e = e.getNext()) {
            if (e == entry) {
               this.modCount++;
               LocalCache.ReferenceEntry<K, V> newFirst = this.removeValueFromChain(
                  first, e, e.getKey(), hash, e.getValueReference().get(), e.getValueReference(), cause
               );
               newCount = this.count - 1;
               table.set(index, newFirst);
               this.count = newCount;
