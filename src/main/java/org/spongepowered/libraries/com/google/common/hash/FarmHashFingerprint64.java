...
final class FarmHashFingerprint64 extends AbstractNonStreamingHashFunction {
   ...
   private static void weakHashLength32WithSeeds(byte[] bytes, int offset, long seedA, long seedB, long[] output) {
      ...
      seedB += Long.rotateRight(var19, 44);
   ...
   private static long hashLength0to16(byte[] bytes, int offset, int length) {
      if (length >= 8) {
         ...
         return hashLength16(length + (a << 3), LittleEndianByteArray.load32(bytes, offset + length - 4) & 4294967295L, mul);
      } else if (length > 0) {
         byte a = bytes[offset];
   ...
   private static long hashLength65Plus(byte[] bytes, int offset, int length) {
      ...
      do {
         long var17 = Long.rotateRight(x + y + v[0] + LittleEndianByteArray.load64(bytes, offset + 8), 37) * -5435081209227447693L;
      ...
      v[0] += w[0];
      w[0] += v[0];
      long var19 = Long.rotateRight(z + y + v[0] + LittleEndianByteArray.load64(bytes, last64offset + 8), 37) * mul;
      ...
      z = Long.rotateRight(x + w[0], 33) * mul;
      ...
      return hashLength16(hashLength16(v[0], w[0], mul) + shiftMix(y) * -4348849565147123417L + var20, hashLength16(v[1], w[1], mul) + z, mul);
   }
}
