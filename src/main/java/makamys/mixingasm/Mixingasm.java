...
public class Mixingasm {
   ...
   public static final Logger LOGGER = LogManager.getLogger("mixingasm");
   ...
   private static List<String> getBadTransformers() {
      ...
      List<String> transformerInclusionPatterns = Stream.of(
         ...
         .flatMap(i -> (Stream<? extends String>)i)
         .filter(Mixingasm::isValidClassPattern)
         .collect(Collectors.toList());
      List<String> transformerExclusionPatterns = readConfig("transformer_exclusion_list.txt")
         .stream()
         .filter(Mixingasm::isValidClassPattern)
         .collect(Collectors.toList());
      ...
      for (IClassTransformer trans : Launch.classLoader.getTransformers()) {
         ...
         if ((
               included = transformerInclusionPatterns.stream().anyMatch(p -> patternMatches(name, p))
   ...
   private static List<String> readConfig(String name) {
      ConfigHelper helper = new ConfigHelper("mixingasm");
   ...
   private static List<String> readConfigLines(File file) {
      try (FileReader fr = new FileReader(file)) {
         return IOUtils.readLines(fr)
            .stream()
            .map(l -> l.contains("#") ? l.substring(0, l.indexOf(35)) : l)
            .map(l -> l.trim())
            .filter(l -> !l.isEmpty())
            .collect(Collectors.toList());
