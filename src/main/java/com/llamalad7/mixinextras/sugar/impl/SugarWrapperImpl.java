...
public class SugarWrapperImpl extends InjectorWrapperImpl {
   ...
   protected SugarWrapperImpl(InjectionInfo wrapper, MixinTargetContext mixin, MethodNode method, AnnotationNode annotation) {
      ...
      method.visibleAnnotations.add(this.originalAnnotation = Annotations.getValue(annotation, "original"));
      this.sugarAnnotations = Annotations.getValue(annotation, "sugars");
      this.generics = new ArrayList<>(GenericParamParser.getParameterGenerics(method.desc, Annotations.getValue(annotation, "signature")));
   ...
   private MethodNode prepareHandler(MethodNode original) {
      ...
      if (handlerInfo == null) {
         ...
         MethodNodeEx newMethod = new MethodNodeEx(
            original.access, MethodNodeEx.getName(original), original.desc, original.signature, original.exceptions.toArray(new String[0]), mixin
         );
