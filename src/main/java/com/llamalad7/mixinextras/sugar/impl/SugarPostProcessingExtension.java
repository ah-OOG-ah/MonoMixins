...
public class SugarPostProcessingExtension implements IExtension {
   ...
   static void enqueuePostProcessing(SugarApplicator applicator, Runnable task) {
      POST_PROCESSING_TASKS.computeIfAbsent(applicator.info.getClassNode().name, k -> new ArrayList<>())
         .add(new SugarPostProcessingExtension.Task(applicator.postProcessingPriority(), task));
   }
   ...
   public void postApply(ITargetClassContext context) {
      ...
      List<SugarPostProcessingExtension.Task> tasks = POST_PROCESSING_TASKS.remove(targetName);
      if (tasks != null) {
         Collections.sort(tasks);
         tasks.forEach(SugarPostProcessingExtension.Task::run);
      }
   }
   ...
   private static class Task implements Comparable<SugarPostProcessingExtension.Task> {
      ...
      public int compareTo(SugarPostProcessingExtension.Task o) {
