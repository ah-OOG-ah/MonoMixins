...
public class ClassGenUtils {
   ...
   static {
      ...
      try {
         Method defineClass = Unsafe.class
         ...
         theDefiner = (name, bytes, scope) -> defineClass.invoke(
            unsafe, name, bytes, 0, bytes.length, scope.lookupClass().getClassLoader(), scope.lookupClass().getProtectionDomain()
         );
