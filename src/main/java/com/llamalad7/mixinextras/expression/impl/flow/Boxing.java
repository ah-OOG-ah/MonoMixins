...
public class Boxing {
   static boolean isBoxing(MethodInsnNode call) {
      ...
      if (unboxingMethod == null) {
         ...
         return call.name.equals(unboxingMethod) || call.name.equals("valueOf") && paramTypes.length == 1 && paramTypes[0].equals(unboxedType);
      }
   }

   public static Type getUnboxedType(Type boxedType) {
      if (boxedType.getSort() != 10) {
         return null;
      } else {
         String var1 = boxedType.getInternalName();
         switch (var1) {
